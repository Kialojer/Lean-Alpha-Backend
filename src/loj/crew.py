...
class JudgeOutput(BaseModel):
    ...
    entry_price: float = Field(description="Suggested entry price based on the data")
    stop_loss: float = Field(description="Strict stop loss level")
    take_profit: float = Field(description="Take profit level ensuring a 1:2 Risk/Reward ratio")
    current_price: float = Field(description="The EXACT current market price fetched from the payload")
    rationale: str = Field(description="A detailed explanation citing SPECIFIC METRICS from the payload (e.g., exact RSI numbers, DEX Volume, Fear/Greed index, or TVL) to justify the trade.")
...
class TradingSystem:
    ...
    def run_pipeline(self, identifier: str, asset_type: str = "CEX", protocol_name: str = None):
        """Executes the dual-agent workflow with Human-in-the-Loop."""
        
        market_data = build_market_payload(identifier, asset_type, protocol_name)
        
        print("\n🧠 Sending data to Judge Agent (gpt-4o-mini)...")
        judge_task = Task(
        ...
        try:
            ...
            print("❌ Error: Failed to parse Judge's output.")
            return

        print(f"\n📊 Judgment Complete! Conviction Score: {score}/100")

        print("\n✍️ Sending setup to Narrator Agent (Gemini Flash)...")
        narrate_task = Task(
        ...
        print("🚀 FINAL OUTPUT FOR X :")
        print("="*55)
        print(final_tweet.raw)
      
        print(" 🟢 Score > 75 : Strong Buy   🟡  Score 50 - 75 : High Risk Buy")
        print(" 🟠 Score 30 - 49 : Sell / Short on Futures 🔴 Score < 30 : Super Risk ")
